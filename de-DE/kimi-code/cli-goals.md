...
preview: true
preview_content: "Mit Zielen sorgst du dafür, dass Kimi Code über mehrere Schritte hinweg auf ein definiertes Ergebnis hinarbeitet."
---
...
Ziele sorgen dafür, dass Kimi Code über mehrere Schritte hinweg auf ein definiertes Ergebnis hinarbeitet. Anders als ein gewöhnlicher Prompt, der vorgibt, was als Nächstes zu tun ist, beschreibt ein Ziel, was wahr werden muss. Verwende `/goal`, wenn die Aufgabe eine klare Ziellinie hat, der nächste sinnvolle Schritt aber davon abhängt, was der Agent während der Arbeit herausfindet – etwa beim Beheben einer Reihe fehlschlagender Tests oder beim Aufspüren der Grundursache eines kaputten Builds.

## Ein Ziel starten

Schreibe das Ziel hinter `/goal`:
...
Vermeide Ziele, die nur eine grobe Richtung vorgeben:
...
Nutze Ziele für Arbeit mit klarer Ziellinie und überprüfbarem Nachweis.
...
Nutze Ziele, wenn eine Aufgabe mehrere Schritte aus Untersuchung und Korrektur erfordern kann.
...
Nutze Ziele für aufeinanderfolgende Arbeit, die ohne weiteren Prompt fortgesetzt werden soll.
...
Das ist hilfreich, wenn du bereits weißt, welche Checks oder Artefakte vorliegen müssen, bevor die Arbeit erledigt ist.

### Wann Ziele ungeeignet sind

Nutze Ziele nicht für weit gefasste Themen oder offene Diskussionen.
...
Nutze Ziele nicht für Aufgaben, die bekanntermaßen unmöglich oder unlösbar sind.
...
Nutze Ziele nicht mit unklaren oder komplizierten Vorgaben.
...
Nutze dieselbe Befehlsoberfläche, um das aktuelle Ziel zu prüfen oder zu steuern:
...
- **pausiert**: Du hast es pausiert, den Schritt unterbrochen, eine Sitzung mit einem aktiven Ziel fortgesetzt oder es ist ein Modell-, Anbieter- oder Laufzeitfehler aufgetreten
...
Schreibe Abbruchbedingungen in das Ziel selbst. `/goal` hat kein eigenes Flag für ein Stopplimit.
...
Verwende `/goal next`, wenn du weitere Arbeit bereit hast, aber das aktuelle Ziel nicht unterbrechen möchtest:
...
Verwalte kommende Ziele interaktiv:
...
Im Verwaltungsmodus blätterst du mit <kbd>↑</kbd> / <kbd>↓</kbd>, wählst mit <kbd>Space</kbd> ein Ziel zum Verschieben aus, ordnest es mit <kbd>↑</kbd> / <kbd>↓</kbd> neu an, bearbeitest mit <kbd>E</kbd>, löschst mit <kbd>D</kbd> und brichst mit <kbd>Esc</kbd> ab. Beim Bearbeiten fügst du mit <kbd>Shift-Enter</kbd> oder <kbd>Ctrl-J</kbd> eine neue Zeile ein und speicherst mit <kbd>Enter</kbd>.

Ist das aktuelle Ziel pausiert, abgebrochen oder blockiert, startet Kimi Code das nächste kommende Ziel nicht. Wenn ein Ziel blockiert und kommende Ziele vorhanden sind, weist dich die TUI darauf hin, dass diese auf den Abschluss warten.
...
Im Berechtigungsmodus `manual` kann die Zielarbeit zur Freigabe eines Tool-Aufrufs pausieren. Für unbeaufsichtigte Arbeit wähle einen Berechtigungsmodus, der zum Risiko des Repositorys und zu den Befehlen passt, die der Agent ausführen darf.
