---
title: "Rechnungen"
slug: "membership-invoice"
order: 9
extract_headings: true
preview: false
---

<SeoMeta
  title="Rechnungen - Kimi Help Center"
  description="Kimi ermöglicht die Selbstbedienungs-Erstellung von Rechnungen für Ihre Mitgliedszahlungen."
/>

# Rechnungen

Kimi ermöglicht die Selbstbedienungs-Erstellung von Rechnungen für Ihre Mitgliedszahlungen.

## Wie erhalte ich eine Rechnung？

Rechnungen werden automatisch an die E-Mail-Adresse gesendet, die bei Ihrem Zahlungsanbieter (Airwallex, Stripe, Apple oder Google) hinterlegt ist.

## Benötigen Sie eine korrigierte Rechnung oder weitere Angaben?
- **E-Mail:** membership@moonshot.ai mit den folgenden Informationen:
- Ursprüngliche Rechnung
- Rechnungsdaten (Firmenname, Adresse, Bundesland/Provinz, E-Mail, USt-IdNr./Steuernummer)

Nutzer in Singapur: Bitte geben Sie zusätzlich Ihre GST-Registrierungsnummer an
