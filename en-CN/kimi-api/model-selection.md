...
preview: true
preview_content: "A Kimi API model selection guide: performance dimensions and Vision billing."
---

<SeoMeta
  title="How to Choose the Right Kimi API Model? - Kimi Help Center"
  description="Choose the Kimi API model that best fits your business needs. This article compares model performance, pricing, and use cases to help you make the best choice."
/>
...
When selecting a model, we recommend evaluating the following dimensions:
...
- **Pricing**: Choose the most cost-effective model based on your budget and call volume.

## Vision Models (Image Understanding)

Vision models support image input and can be used for image descriptions, OCR, chart interpretation, and similar scenarios:

- Each image is billed at a fixed **1024 tokens**, regardless of image size or resolution.
- Common image formats are supported, including JPEG, PNG, WebP, and more.
- Images can be passed in via URL or Base64 encoding.

## Currently Unsupported Capabilities

- **Video multimodality**: Direct input and understanding of video files are not supported yet.
- **PPT Generation API**: The PPT generation feature is not yet available through API.
- **Deep Research API**: The Deep Research feature is not yet available through API.
