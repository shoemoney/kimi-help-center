---
title: "Kimi Business"
slug: "kimi-business"
order: 1
extract_headings: false
preview: false
---

<SeoMeta
  title="Kimi Business - Centre d'aide Kimi"
  description="Découvrez le forfait entreprise Kimi Business, sa tarification, la gestion d'équipe et ses fonctionnalités professionnelles. Conçu pour les équipes et les entreprises, avec protection de la confidentialité des données et assistance dédiée."
/>

# Kimi Business

Kimi Business est une solution d'espace de travail intelligent conçue pour les équipes d'entreprise. Elle offre l'ensemble des capacités d'IA tout en garantissant la sécurité des données de l'entreprise et en facilitant la collaboration au sein des équipes.

<Callout type="tip">
Kimi Business propose des abonnements annuels à partir de 5 sièges, parfaitement adaptés aux petites et moyennes entreprises ayant des besoins de collaboration en équipe.
</Callout>

## Comment acheter

| Taille de l'entreprise | Parcours recommandé | Comment acheter | Contact commercial |
|---|---|---|---|
| 1–20 | Acheter le plan Business | [Acheter en ligne](https://www.kimi.com/membership/pricing?from=help&tab=business) | sales@kimi.ai |
| 20+ | Passer des commandes en ligne séparées | [Acheter en ligne](https://www.kimi.com/membership/pricing?from=help&tab=business) | sales@kimi.ai |
| Utilisation à grande échelle d'API Tokens | Acheter l'API sur la plateforme ouverte | [Kimi API Platform](https://platform.kimi.ai/) | [Contacter le service commercial (formulaire)](https://platform.kimi.ai/contact-sales) |

## Tarification

| Élément | Détails |
|------|---------|
| Prix | 600 $/siège/an (environ 50 $/siège/mois) |
| Achat minimum | 5 sièges, à partir de 3 000 $/an |
| Facturation | Abonnement annuel, ajout de sièges possible en cours d'abonnement |

Les sièges supplémentaires achetés en cours d'abonnement sont facturés au prorata des jours restants et expirent à la même date que l'abonnement initial.

## Avantages principaux

- Commande minimale de 5 sièges
- Vos contenus ne sont pas utilisés pour l'entraînement des modèles par défaut
- Jusqu'à 1 million de tokens de contexte pour les longues conversations
- Débloquez toutes les fonctionnalités avancées : Kimi Work / Swarm / Dashboard / Plugins
- Kimi Code disponible
- Assistance technique dédiée

### Fonctionnalités d'entreprise

- **Protection de la confidentialité des données** : les données de l'entreprise ne sont pas utilisées pour l'entraînement des modèles, ce qui garantit la sécurité des informations professionnelles
- **Gestion des membres** : les administrateurs peuvent inviter et retirer des membres, et répartir les sièges en toute flexibilité
- **Assistance technique dédiée** : les clients entreprise bénéficient d'une assistance technique dédiée

## Gestion des membres

### Rôles et autorisations

| Rôle | Autorisations |
|------|-------------|
| Administrateur | Acheter des sièges, générer des liens d'invitation, gérer les membres, consulter les commandes |
| Membre | Profiter des avantages de l'entreprise, consulter ses crédits personnels |

Le rôle d'administrateur est automatiquement attribué au compte qui souscrit en premier à l'abonnement entreprise.

### Inviter des membres

Les administrateurs peuvent générer des liens d'invitation dont la durée de validité est configurable à 2 ou 7 jours. Une fois l'invitation acceptée, les membres rejoignent l'organisation et accèdent aux avantages de l'entreprise.

<Callout type="info">

Chaque utilisateur peut rejoindre jusqu'à 10 organisations d'entreprise. Un membre occupe 1 siège dès qu'il rejoint l'organisation, et un même utilisateur n'occupe qu'1 seul siège au sein d'une organisation. Pour inviter de nouveaux membres, l'organisation doit disposer de sièges non attribués.

</Callout>

### Statut des sièges

| Statut | Description |
|--------|-------------|
| Total des sièges | Tous les sièges non expirés achetés par l'organisation |
| Non attribués | Sièges actuellement inoccupés, disponibles pour inviter de nouveaux membres |
| En attente de libération | Sièges occupés par des membres retirés au cours du cycle de facturation en cours ; ces sièges ne peuvent pas être attribués à de nouveaux membres et sont libérés au cycle suivant |

### Retirer des membres

- Les membres retirés perdent immédiatement l'accès à l'espace de travail de l'entreprise
- Les membres retirés perdent immédiatement les avantages Kimi Business et ne peuvent plus utiliser les crédits de l'entreprise pour la période en cours
- Les sièges des membres retirés passent au statut « En attente de libération » et sont libérés au cycle de facturation suivant

## Espace de travail

Kimi Business introduit le concept d'espace de travail afin d'assurer une isolation totale entre les données personnelles et professionnelles.

### Types d'espaces de travail

- **Espace de travail personnel** : avantages d'abonnement personnel, historique des conversations et paramètres
- **Espace de travail d'entreprise** : avantages d'abonnement entreprise et données liées à l'entreprise

Lors du changement d'espace de travail, la page se rafraîchit automatiquement pour charger les données et la configuration des avantages de l'espace sélectionné.

<Callout type="warning">

Compatibilité des plateformes : le Web et le client de bureau prennent en charge les fonctionnalités entreprise de Kimi Business. L'application mobile ne prend actuellement en charge que l'abonnement personnel ; les fonctionnalités d'entreprise n'y sont pas disponibles.

</Callout>

## Règles de distribution des avantages

### Règles d'activation

- **Activation immédiate** : les membres bénéficient des avantages de la période en cours dès qu'ils acceptent l'invitation
- **Crédits complets de la période en cours** : les membres qui rejoignent l'organisation avec un siège non attribué ou nouvellement ajouté reçoivent l'intégralité des crédits de la période en cours
- **Alignement sur le cycle** : les avantages de l'entreprise sont calculés de manière uniforme selon le cycle d'abonnement et renouvelés chaque mois

### Règles d'expiration

- **Retrait d'un membre** : les membres retirés perdent immédiatement l'accès à l'espace de travail de l'entreprise ainsi que les avantages Kimi Business ; les nouveaux avantages cessent au cycle suivant
- **Expiration de l'abonnement** : tous les avantages des membres sont suspendus à l'expiration de l'abonnement de l'organisation, puis rétablis lors du renouvellement
