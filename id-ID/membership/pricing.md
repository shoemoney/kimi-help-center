---
title: "Detail harga"
slug: "membership-pricing"
order: 4
extract_headings: true
preview: false
---

<SeoMeta
  title="Detail harga - Pusat Bantuan Kimi"
  description="Kimi menawarkan lima tingkat keanggotaan — termasuk paket gratis — masing-masing tersedia sebagai langganan bulanan atau tahunan. Penagihan tahunan menghemat hingga $480/tahun."
/>

# Detail harga

Kimi menawarkan lima tingkat keanggotaan — termasuk paket gratis — masing-masing tersedia sebagai langganan bulanan atau tahunan. Penagihan tahunan menghemat hingga **$480/tahun**.

## Ringkasan paket

| Fitur | Adagio (Gratis) | Moderato ($19/bln) | Allegretto ($39/bln) | Allegro ($99/bln) | Vivace ($199/bln) |
|---------|---------------|-------------------|---------------------|-------------------|-------------------|
| Kredit Agent* | 6 | 60 | 150 | 360 | 720 |
| Tugas serentak Agent | 1 | 2 | 2 | 4 | 4 |
| Prioritas kecepatan Agent | — | 4× | 4× | 4× | 4× |
| Agent Swarm (beta) | — | 25 penggunaan | 50 penggunaan | 120 penggunaan | 240 penggunaan |
| Subtugas serentak Agent Swarm | — | 2 | 4 | 4 | 8 |
| Kredit Kimi Code | — | 1× | 5× | 15× | 30× |
| Kimi Claw | — | — | ✅ | ✅ | ✅ |
| Kimi Claw Android | — | — | ✅ | ✅ | ✅ |
| Basis data profesional | 200 panggilan | 2.000 panggilan | 5.000 panggilan | 12.000 panggilan | 24.000 panggilan |

\* Nilai perkiraan berdasarkan konsumsi token tugas pada umumnya. Kredit bulanan dikonversi menjadi jumlah tugas setara untuk fitur yang sama; hanya sebagai referensi.

## Harga bulanan vs. tahunan

| Paket | Bulanan | Tahunan (per bulan) | Total tahunan | Penghematan |
| --- | --- | --- | --- | --- |
| Moderato | $19/bln | $15/bln | $180/thn | $48/thn |
| Allegretto | $39/bln | $31/bln | $372/thn | $96/thn |
| Allegro | $99/bln | $79/bln | $948/thn | $240/thn |
| Vivace | $199/bln | $159/bln | $1.908/thn | $480/thn |

## Alokasi kredit

Fitur Agent dan fitur keanggotaan Kimi lainnya berbagi satu kumpulan kredit; Kimi Code memiliki kumpulan kredit terpisahnya sendiri. Tingkat yang lebih tinggi mencakup kumpulan kredit yang proporsional lebih besar, sehingga mendukung alur kerja yang lebih sering dan kompleks. Lihat [Aturan Pembaruan & Penggunaan Kredit](/membership/update-rules) untuk detailnya.
