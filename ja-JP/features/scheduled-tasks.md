...
タスクの作成には**手動作成**と**会話で作成**の2つのモードがあります。手動作成では、デフォルトで K2.6 モデルを使ってスケジュールタスクを作成します。会話で作成では、新しい会話を始めてからモデルを切り替え、プロンプトを入力してタスクを作成できます。タスクの実行後には結果の会話が生成され、その中でモデルを切り替えて追加質問を続けたり、「/」を入力してプラグインや Skills を呼び出したりできます。
