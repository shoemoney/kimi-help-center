...
<Callout type="info">
Kimi Code CLI は [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) による IDE 連携に対応しており、エディタ内でそのまま AI 支援コーディングを利用できます。
</Callout>
...
Paseo の汎用 ACP アダプタはログインフローを起動しないため、先にターミナルでログインを完了してください（[前提条件](#prerequisites)を参照）。完了していない場合、セッション作成は `Authentication required` で失敗します。
