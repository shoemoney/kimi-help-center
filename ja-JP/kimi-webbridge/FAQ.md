...
<SeoMeta
  ...
  description="Kimi WebBridge のインストール、接続、複数ブラウザやローカルエージェントでの利用に関するよくある質問です。"
  ogType="article"
/>
...
- **その他のローカルエージェント**: エージェントで次のコマンドを再送信し、実行後にエージェントを再起動してください。
...
4. ブラウザを再起動して、もう一度試す
5. WebBridge が正常に動作する場合は、他の拡張機能を1つずつ有効に戻し、競合している拡張機能を特定する


### WebBridge からログイン情報が漏れることはありますか？

いいえ。すべての実行処理はお使いのデバイス上でローカルに行われます。ログイン状態やウェブページの内容が端末の外へ送信されることはありません。エージェントがアクセスできるのは、あなたが許可した操作結果だけです。

### 操作が失敗するのはなぜですか？

ページ構造が複雑だったり、動的読み込みが行われていたりすると、操作に失敗することがあります。指示を簡潔にするか、実行前にまずエージェントにスクリーンショットを撮らせて、ページの状態を確認してください。
...
WebBridge は、Claude Code、Codex、Cursor、Kimi Code、Hermes Claw など、すべてのローカルエージェントに対応しています。

### エージェントによって設定方法は異なりますか？

設定コマンドはすべて共通です。1つの curl コマンドを実行するだけでインストールできます。詳細は各エージェントの Skill 設定を参照してください。
