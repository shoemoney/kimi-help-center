...
파일 업로드 자체는 무료입니다. 다만 대화에서 업로드한 파일을 참조하면 파일 내용이 토큰으로 파싱되어 입력 토큰으로 과금됩니다. 파일이 클수록 더 많은 토큰이 생성됩니다.
...
- 응답의 `finish_reason` 필드를 확인합니다. `length`는 토큰 제한 때문에 출력이 잘렸다는 뜻이고, `stop`은 정상적으로 완료되었다는 뜻입니다.
