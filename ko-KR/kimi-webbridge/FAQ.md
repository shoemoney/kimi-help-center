...
Kimi WebBridge가 Kimi 브라우저 확장 프로그램으로 이름이 변경되었고, 브라우저 사이드바 진입점, 웹 작업 녹화, 웹페이지를 분해해 지시문을 생성하는 기능이 새로 추가되었습니다. Local Agent에서 확장 프로그램을 호출하는 방식은 그대로 유지되므로 계속 사용할 수 있으며, 새로 추가된 사이드바 진입점은 Kimi 계정으로 로그인한 후 사용할 수 있습니다.
