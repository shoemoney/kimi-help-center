...
<SeoMeta
  title="Integração com IDE - Central de Ajuda do Kimi"
...
<Callout type="info">
O Kimi Code CLI oferece integração com IDEs por meio do [Agent Client Protocol (ACP)](https://agentclientprotocol.com/), permitindo que você use programação assistida por IA diretamente dentro do seu editor.
</Callout>
...
O adaptador ACP genérico do Paseo não conduz o fluxo de login, então conclua primeiro o login pelo terminal (consulte [Pré-requisitos](#prerequisites)) — caso contrário, a criação da sessão falhará com `Authentication required`.
