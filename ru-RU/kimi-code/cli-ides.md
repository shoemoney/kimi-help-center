...
Универсальный ACP-адаптер Paseo не запускает сценарий входа, поэтому сначала выполните вход в терминале (см. [Предварительные требования](#prerequisites)) — иначе создание сессии завершится ошибкой `Authentication required`.
