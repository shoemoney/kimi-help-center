---
title: "Kimi Business"
slug: "kimi-business"
order: 1
extract_headings: false
preview: false
---

<SeoMeta
  title="Kimi Business - Kimi Yardım Merkezi"
  description="Kimi Business kurumsal planını; fiyatlandırma, ekip yönetimi ve kurumsal özellikleriyle birlikte keşfedin. Veri gizliliği koruması ve özel destekle ekipler ve işletmeler için tasarlandı."
/>

# Kimi Business

Kimi Business, kurumsal ekipler için tasarlanmış akıllı bir çalışma alanı çözümüdür. Kurumsal veri güvenliğini sağlarken eksiksiz yapay zekâ yeteneklerini sunar ve ekip iş birliğini destekler.

<Callout type="tip">
Kimi Business, en az 2 koltuktan başlayan yıllık abonelikler sunar (satın alma başına en fazla 150 koltuk); ekip iş birliği ihtiyacı olan küçük ve orta ölçekli işletmeler için idealdir.
</Callout>

## Nasıl satın alınır

| Şirket büyüklüğü | Önerilen yol | Nasıl satın alınır | Satış iletişimi |
|---|---|---|---|
| 1–20 | Business planını satın alın | [Çevrimiçi satın al](https://www.kimi.com/membership/pricing?from=help&tab=business) | sales@kimi.ai |
| 20+ | Ayrı çevrimiçi siparişler verin | [Çevrimiçi satın al](https://www.kimi.com/membership/pricing?from=help&tab=business) | sales@kimi.ai |
| Büyük ölçekli API Tokens kullanımı | API'yi açık platformdan satın alın | [Kimi API Platform](https://platform.kimi.ai/) | [Satışla iletişime geçin (form doldurun)](https://platform.kimi.ai/contact-sales) |

## Fiyatlandırma

| Öğe | Ayrıntılar |
|------|---------|
| Fiyat | 600 $/koltuk/yıl (yaklaşık 50 $/koltuk/ay) |
| Minimum Satın Alma | 2 koltuk, yılda 1.200 $'dan başlayan; satın alma başına en fazla 150 koltuk |
| Faturalandırma | Yıllık abonelik, abonelik süresi boyunca koltuk eklemeyi destekler |

Abonelik süresi içinde satın alınan ek koltuklar, kalan gün sayısına göre orantılı olarak ücretlendirilir ve mevcut aboneliğle aynı sona erme tarihine sahip olur.

## Temel Avantajlar

- En az 2 koltukluk minimum sipariş
- İçeriğiniz varsayılan olarak model eğitiminde kullanılmaz
- Uzun konuşmalar için 1 milyon tokena kadar bağlam
- Tüm gelişmiş özelliklerin kilidini açın: Kimi Work / Swarm / Dashboard / Plugins
- Kimi Code kullanılabilir
- Özel teknik destek

### Kurumsal Özellikler

- **Veri Gizliliği Koruması**: Kurumsal veriler model eğitiminde kullanılmaz; iş bilgilerinizin güvenliği sağlanır
- **Üye Yönetimi**: Yöneticiler üye davet edip kaldırabilir ve koltukları esnek biçimde dağıtabilir
- **Özel Teknik Destek**: Kurumsal müşteriler özel teknik destek alır

## Üye Yönetimi

### Roller ve İzinler

| Rol | İzinler |
|------|-------------|
| Yönetici | Koltuk satın alma, davet bağlantısı oluşturma, üye yönetme, siparişleri görüntüleme |
| Üye | Kurumsal avantajları kullanma, kişisel kredileri görüntüleme |

Yönetici rolü, kurumsal aboneliği ilk satın alan hesaba otomatik olarak atanır.

### Üye Davet Etme

Yöneticiler, 2 veya 7 günlük yapılandırılabilir sona erme süreleriyle davet bağlantıları oluşturabilir. Üyeler daveti kabul ettikten sonra organizasyona katılabilir ve kurumsal avantajlara erişebilir.

<Callout type="info">

Her kullanıcı en fazla 10 kurumsal organizasyona katılabilir. Üyeler katıldıklarında 1 koltuk işgal eder ve aynı kullanıcı bir organizasyon içinde yalnızca 1 koltuk kaplar. Yeni üye davet edebilmek için organizasyonların kullanılabilir, atanmamış koltuklarının olması gerekir.

</Callout>

### Koltuk Durumu

| Durum | Açıklama |
|--------|-------------|
| Toplam Koltuk | Organizasyonun satın aldığı, süresi dolmamış tüm koltuklar |
| Atanmamış | Şu anda kullanılmayan, yeni üye davet etmek için müsait koltuklar |
| Serbest Bırakılmayı Bekleyen | Mevcut faturalandırma döngüsü içinde kaldırılan üyeler tarafından işgal edilen koltuklar; bu koltuklar yeni üyelere atanamaz ve bir sonraki döngüde serbest bırakılır |

### Üye Kaldırma

- Kaldırılan üyeler, kurumsal çalışma alanına erişimini anında kaybeder
- Kaldırılan üyeler, Kimi Business avantajlarını anında kaybeder ve mevcut dönem için kurumsal kredileri artık kullanamaz
- Kaldırılan üyelerin koltukları "Serbest Bırakılmayı Bekleyen" durumuna geçer ve bir sonraki faturalandırma döngüsünde serbest bırakılır

## Çalışma Alanı

Kimi Business, kişisel ve kurumsal veriler arasında tam bir ayrım sağlamak için çalışma alanı kavramını sunar.

### Çalışma Alanı Türleri

- **Kişisel Çalışma Alanı**: Kişisel üyelik avantajları, sohbet geçmişi ve ayarlar
- **Kurumsal Çalışma Alanı**: Kurumsal üyelik avantajları ve kuruma ilişkin veriler

Çalışma alanları arasında geçiş yapıldığında sayfa otomatik olarak yenilenir ve seçilen çalışma alanına ait veriler ile avantaj yapılandırmaları yüklenir.

<Callout type="warning">

Platform Desteği: Web ve masaüstü istemciler Kimi Business kurumsal özelliklerini destekler. Mobil uygulama şu anda yalnızca kişisel üyeliği destekler; kurumsal özellikler burada kullanılamaz.

</Callout>

## Avantaj Dağıtım Kuralları

### Etkinleştirme Kuralları

- **Anında Etkinleştirme**: Üyeler, daveti kabul eder etmez mevcut dönem avantajlarını hemen alır
- **Tam Dönem Kredileri**: Atanmamış veya yeni eklenen bir koltukla katılan üyeler, mevcut döneme ait kredilerin tamamını alır
- **Döngü Hizalaması**: Kurumsal avantajlar, abonelik döngüsüne göre tek tip olarak hesaplanır ve aylık olarak yenilenir

### Sona Erme Kuralları

- **Üye Kaldırma**: Kaldırılan üyeler, kurumsal çalışma alanı erişimini ve Kimi Business avantajlarını anında kaybeder; yeni avantajlar bir sonraki döngüde durdurulur
- **Abonelik Sona Ermesi**: Organizasyon aboneliği sona erdiğinde tüm üye avantajları askıya alınır; yenileme yapıldığında geri yüklenir
