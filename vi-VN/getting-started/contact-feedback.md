---
title: "Liên hệ & phản hồi"
slug: "contact-feedback"
order: 8
extract_headings: false
preview: false
---

<SeoMeta
  title="Liên hệ & phản hồi - Trung tâm trợ giúp Kimi"
  description="Tìm hiểu cách liên hệ với đội ngũ Kimi để báo lỗi, gửi báo cáo sự cố và đề xuất tính năng. Phản hồi của bạn là cơ sở quan trọng giúp chúng tôi cải thiện sản phẩm."
/>

# Liên hệ & phản hồi

**Hỏi: Tôi có thể gửi phản hồi ở đâu?**

Chọn kênh phù hợp dựa trên loại phản hồi của bạn:

| Loại phản hồi | Kênh |
|--------------|---------|
| Sự cố sản phẩm | Trên web: nhấp vào ảnh đại diện (góc dưới bên trái) → User Feedback → Help & Feedback. Trong ứng dụng: Hồ sơ → Cài đặt → Phản hồi |
| Phản hồi về chất lượng trò chuyện | Sau một cuộc trò chuyện, nhấp vào nút **👎** và mô tả loại vấn đề cùng sai lệch cụ thể |
| Khiếu nại tài khoản / báo cáo | [support@moonshot.ai](mailto:support@moonshot.ai) |
| Liên hệ truyền thông | [pr@kimi.com](mailto:pr@kimi.com) |
| Hỗ trợ Kimi API | Đăng nhập vào [Kimi Open Platform](https://platform.kimi.ai/) để xem thông tin liên hệ chăm sóc khách hàng và cộng đồng nhà phát triển |
