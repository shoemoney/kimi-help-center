...
preview: true
preview_content: "通过 ACP 将 Kimi Code CLI 集成到 Zed、JetBrains IDE 和 Paseo 中。"
---

<SeoMeta
  title="在 IDE 中使用 - Kimi 帮助中心"
  description="通过 ACP 协议将 Kimi Code CLI 集成到 IDE 中。Zed、JetBrains IDE 和 Paseo 的配置指南，以及常见问题排查。"
/>
...
JetBrains 这一侧对 `command` 字段处理较严格——务必填写**绝对路径**，可以在终端执行 `which kimi` 拿到。
...
保存后，AI 聊天的 Agent 选择器里就会出现 `Kimi Code CLI`。
...
- **MCP 工具看不到**：参考 `kimi acp` 中的能力表确认 IDE 配的 MCP 传输类型是否被支持。当前 Kimi Code CLI 的 ACP 适配层支持 `http`、`stdio` 两种传输方式，`sse` 与 `acp` 类型会被静默丢弃并在日志中给出 warn。

## 下一步

- [kimi acp 参考](/kimi-code-cli/reference/kimi-acp) — ACP 能力矩阵和方法覆盖详情
- [kimi 命令参考](/kimi-code-cli/reference/kimi-command) — 完整子命令列表
