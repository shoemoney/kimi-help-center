...
Kimi 提供四档会员订阅套餐，满足从个人日常到专业重度使用的不同需求。所有套餐的会员权益共享一个额度池，Agent 系列、Kimi Code 等各项功能均从同一额度池扣除。Chat 不消耗额度。额度可自由分配使用。
...
我们进行了权益升级，已经为你打通了权益额度。现在，Agent、Kimi Code、PPT 等各项会员功能共享一个额度池，按实际使用量计费。具体可参考[会员额度说明](https://www.kimi.com/membership-credits)。
