...
Kimi 提供四档会员订阅套餐，满足从个人日常到专业重度使用的不同需求。所有套餐的会员权益包含两个额度池：Kimi Code 使用单独额度池，其他会员功能共享额度池。Chat 不消耗额度。额度可自由分配使用。
...
我们进行了权益升级，已经为你打通了权益额度。现在，Agent、PPT 等其他会员功能共享一个额度池，按实际使用量计费。Kimi Code 使用单独额度池。具体可参考[会员额度说明](https://www.kimi.com/membership-credits)。
