...
<SeoMeta
  ...
  description="了解 Kimi 的三档模型（K2.6 Fast、K3 Max、K3 集群 Max）、内置工具与多模态能力，快速上手对话与任务执行。"
/>
...
| 模型 | 思考强度 | 擅长场景 |
|------|----------|----------|
| **K2.6 Fast** | 标准 / 进阶 | 快速对话与问答，响应更快 |
| **K3 Max** | Max | 对话与 Agent 任务，综合能力最强（推荐） |
| **K3 集群 Max** | Max | 海量搜索、批量处理，一次性完成大规模任务 |

<Callout type="tip">
K2.6 Fast 不消耗会员额度；K3 Max 与 K3 集群 Max 按额度计费，详见[会员权益说明](https://www.kimi.com/help/membership/membership-overview)。
</Callout>
...
| Plugin 插件 | 将外部工具、应用与专业数据库接入 Kimi，调用其能力完成任务 |
| Image Search | 以文搜图、以图搜图，快速定位视觉内容 |
| Image Generation | 图像生成（K3 内置），可直接根据描述生成图像 |
| IPython | 执行 Python 代码，进行数据与可视化分析 |
| Memory | 智能记忆功能，记录你的个人习惯与偏好设置 |

## 视觉与多模态能力

Kimi 三档模型均支持图片、视频、PDF/Word/Excel 等办公文档的全场景理解，采用**原生多模态架构**（非外挂 OCR），可直接解析文档内容并执行中高阶办公操作（如编写 Excel 公式、生成 PPT 大纲/VBA 代码）。其中 K3 原生视觉能力进一步增强，配合 100 万 token 上下文可处理更长、更复杂的多模态材料。
...
1. **生图能力**：K3（Max / 集群）内置图像生成工具，可直接根据描述生成图像；K2.6 Fast 仅支持搜索图片，不支持直接生成图像。
2. **文件产出**：K3 可端到端产出可编辑文档（如 `.pptx`、`.docx`、`.xlsx`、`.pdf`）等复杂结果；K2.6 Fast 偏快速问答，仅输出文本内容（如 PPT 大纲、Word 正文、代码等）。
3. 如需生图、生成复杂可编辑文档或执行多步任务，请选择 **K3 Max** 或 **K3 集群 Max**。
