...
Kimi 提供五種會員等級（含免費方案），可依你的需求彈性搭配，從日常效率提升到進階工作流程都能支援。所有方案的各項會員功能（Agent 系列、Kimi Code 等）都**共用同一個額度池**，並依實際 token 消耗計算。
...
- **共用額度池** — 各項會員功能（Agent 系列、Kimi Code 等）共用同一個額度池，並依 token 用量計量。
- **每月重置** —額度 會在每個計費週期開始時重置。
- **扣抵順序** — 先使用贈送額度，再使用方案額度s。
