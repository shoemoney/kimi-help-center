...
Kimi 提供五種會員等級（含免費方案），可依你的需求彈性搭配，從日常效率提升到進階工作流程都能支援。所有方案都包含 **兩個額度池**：Kimi Code 使用獨立的額度池；其他會員功能共用同一個額度池。
...
- **兩個額度池** — Kimi Code 使用獨立的額度池；其他會員功能共用同一個額度池，並依 token 用量計量。
